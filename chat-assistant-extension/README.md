...
插件在用户允许连接本地服务后，通过 WebSocket 接收 FastAPI 生成的
`introduction_ready` 任务，只在
`https://www.zhipin.com/web/geek/chat*` 页面定位联系人、切换聊天并回填消息。

点击浏览器工具栏中的插件图标，可以分别控制“连接本地服务”和“自动发送”：

- 连接本地服务默认关闭：插件不创建 WebSocket，也不执行心跳或自动重连。
- 开启连接：插件才会连接本地 FastAPI；后端暂未启动时按 1、2、5、10 秒间隔重连，
  此后保持每 10 秒重试。
- 再次关闭连接：立即取消重连和心跳，并主动关闭现有 WebSocket。
...
5. 打开并刷新 BOSS 消息页面。
6. 点击插件图标，将“连接本地服务”设为“是”；首次安装默认为“否”。
7. 根据需要选择是否自动发送；首次安装同样默认为“否”。

后台默认连接 `ws://127.0.0.1:8000/ws/chat-assistant-extension`。端口变化时同步修改
`config.js` 的 `websocketUrl`。只有“连接本地服务”开关为“是”时才会尝试建立连接。
