...
插件等待 FastAPI 响应的默认上限是 120 秒，以覆盖通常耗时 60～90 秒的 Coze
...
3. 选择“加载已解压的扩展程序”，选择本目录 `job-scanner-extension`。
...
为避免岗位切换过快导致 SPA 重载，插件默认在首次点击前等待 1.8～3 秒，后续两次岗位点击至少间隔 1～2 秒，并在详情标题更新后额外等待 0.5～1 秒让描述和 HR 区域稳定。所有等待均可被“停止”中断；具体数值集中在 `config/constants.js` 中。
...
此项目是本地测试工具。自动沟通会对真实招聘方产生外部动作；海投模式固定返回 71 分并会触发沟通。请先关闭海投，用确定低于 `MATCH_THRESHOLD` 的测试结果确认 selector 和页面状态，再小范围验证沟通流程。请遵守网站条款并控制使用频率。
